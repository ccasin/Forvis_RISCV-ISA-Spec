...
prettyMState :: PolicyPlus -> MState -> Doc
prettyMState pplus (M (m1, p1)) =
    let ppol = policy pplus in
    P.vcat [ P.text "PC:" <+> pretty pplus (f_pc m1) (p_pc p1)
           , P.text "Registers:" $$ P.nest 2 (pretty pplus (f_gprs m1) (p_gprs p1))
           , P.text "Memory:" $$ P.nest 2 (pretty pplus (f_mem m1) (p_mem p1))
           ]
--
...
printTrace pplus tr1 = putStrLn $ P.render $ prettyTrace pplus tr1

prettyTrace :: PolicyPlus -> [(Machine_State, PIPE_State)] -> Doc
prettyTrace pplus [] = P.empty
--prettyTrace pplus [(m1,p1)] = prettyMState pplus (M (m1,p1))
prettyTrace pplus ((m1,p1):tr) =
  P.vcat [prettyMState pplus (M (m1,p1)), prettyTrace pplus tr]
